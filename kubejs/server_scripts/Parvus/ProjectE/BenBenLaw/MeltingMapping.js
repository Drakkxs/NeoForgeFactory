...
// requires: casting
// @ts-check
// Automatically map Melting recipes to ProjectE conversions.

// Immediately Invoked Function Expression to prevent polluting the global namespace
(() => {
    let debug = false; // Want some debug?
    const filePath = 'kubejs/data/projecte/pe_custom_conversions/generated_melting.json';
    ...
    ServerEvents.recipes(event => {
        const name = "melting";
        const comment = "BBL Melting recipes mapped to ProjectE conversions.";
        const conversions = [];
        ...
        event.forEachRecipe({
            ...
            if (debug) {
                console.log(`Found BBL Melting recipe: ${recipe.id}`);
